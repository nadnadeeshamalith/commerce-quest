...
// ප්‍රශ්න බැංකුව (Professional A/L Questions for Paper 1)
const fullQuestionBank = [
  ...
  { paperId: 1, id: 7, subject: "BS", question: "ව්‍යවසායකයෙකුගේ මූලික ලක්ෂණයක් වන්නේ?", options: ["අවදානම් මඟ හැරීම", "නවෝත්පාදනය", "ප්‍රාග්ධනය නොමැතිකම", "අන් අයගේ උපදෙස් මත පමණක් ක්‍රියා කිරීම"], answer: 1, explanation: "අවදානම් දරමින් නව දෑ සොයා යාම (නවෝත්පාදනය) ව්‍යවසායකත්වයේ මූලික ලක්ෂණයකි." },
...
export default function App() {
  ...
  // Fetch Public Leaderboard with Safety Check
  useEffect(() => {
    if (!user || !db) return;
  ...
  // Fetch User History with Safety Check
  useEffect(() => {
    if (!user || !db) return;
  ...
  const selectPaper = (paperId) => {
    ...
    if (paperQuestions.length === 0) {
      alert("මෙම ප්‍රශ්න පත්‍රය තවමත් සකස් කර නැත.");
      return;
    }
  ...
  const handleAnswer = (idx) => {
    ...
    const correct = idx === currentQuestions[currentIndex].answer;
  ...
  const saveScore = async () => {
    ...
    if (!user) {
      alert("දෝෂයක්: පරිශීලක ගිණුම (User Auth) සම්බන්ධ වී නැත!\n\nකරුණාකර Firebase Console එකට ගොස් Authentication -> Sign-in method යටතේ 'Anonymous' කියන එක Enable කර Save කර ඇතිදැයි පරීක්ෂා කරන්න. ඉන්පසු වෙබ් පිටුව Refresh (F5) කරන්න.");
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 p-6">
        <div className="bg-slate-900 border border-blue-500/30 p-8 rounded-[2rem] text-center max-w-md shadow-2xl">
          ...
          <h2 className="text-2xl font-black text-white mb-4 uppercase text-white">Setup Required</h2>
          <p className="text-slate-400 leading-relaxed mb-6">
            ඔබේ Firebase Config දත්ත <b>App.jsx හි 18 වන පේළියේ</b> ඇතුළත් කරන්න.
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black uppercase tracking-widest hidden sm:block text-white">Commerce Pro</h1>
          </div>
        ...
        {gameState === 'start' && (
          <div className="text-center py-12 animate-in zoom-in duration-500">
            ...
            <h2 className="text-5xl md:text-6xl font-black mb-6 tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-emerald-400 drop-shadow-lg">
              CHALLENGE MODE
            </h2>
        ...
        {gameState === 'select_paper' && (
          <div className="animate-in fade-in duration-500">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-2xl font-black uppercase text-white">Select Paper</h2>
            </div>
            <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-8 gap-3">
              {Array.from({ length: 40 }, (_, i) => (
                <button
                  ...
                  className="aspect-square bg-slate-900 border border-slate-800 rounded-xl flex flex-col items-center justify-center hover:bg-blue-600 hover:border-blue-400 transition-all group text-white"
                >
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4">
             <div className="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-xl">
                ...
                <div className="text-center">
                  <span className="block text-[10px] text-slate-500 font-bold uppercase tracking-widest text-slate-500">Paper {selectedPaper}</span>
             ...
             <div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-2xl">
                  ...
                  {currentQuestions[currentIndex].options.map((opt, i) => (
                    <button
                      ...
                      className={`w-full text-left p-5 rounded-2xl border-2 transition-all flex justify-between items-center
                        ${selectedOption === i 
                          ? (isCorrect ? 'border-emerald-500 bg-emerald-500/10 text-emerald-100' : 'border-rose-500 bg-rose-500/10 text-rose-100') 
                          : 'border-slate-800 bg-slate-950/50 hover:border-slate-600 hover:bg-slate-800 text-slate-300'}
                        ${showFeedback && i === currentQuestions[currentIndex].answer ? 'border-emerald-500 bg-emerald-500/10 text-emerald-100' : ''}
                      `}
                    >
                ...
                {showFeedback && (
                  <div className="p-8 border-t border-slate-800 bg-slate-950/30">
                    <p className="text-slate-400 text-sm mb-6 italic leading-relaxed">{currentQuestions[currentIndex].explanation}</p>
                    <button onClick={nextStep} className="w-full bg-white text-slate-950 font-black py-4 rounded-xl flex items-center justify-center gap-2 uppercase tracking-widest">
        ...
        {gameState === 'result' && (
          <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 text-center animate-in zoom-in shadow-2xl">
            ...
            <h2 className="text-3xl font-black mb-2 uppercase text-white">විශිෂ්ටයි!</h2>
        ...
        {gameState === 'history' && (
          <div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden animate-in fade-in">
             <div className="p-8 border-b border-slate-800 flex justify-between items-center bg-slate-800/20">
               ...
               <button onClick={() => setGameState('start')} className="p-2 bg-slate-950 border border-slate-800 rounded-lg text-white"><X /></button>
             </div>
